...
export class Player extends TransformNode {
    ...
    private _updateFromInputController() {
        ...
        this._moveDirection=this._moveDirection.scaleInPlace(Player.PLAYER_SPEED) //玩家速度
    ...
    private _updateGroundDetection(){ //检测是否在地面上 如果没在地面上 施加重力
        if (!this._isGrounded()) {
            //重力添加
            this._gravity = this._gravity.addInPlace(Vector3.Down().scale(Player.PLAYER_GRAVITY));
